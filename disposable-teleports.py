def checkio(teleports_string):
    route = []
    connections = {}
    for src, tgt in teleports_string.split(','):
        try:
            connections[src].append(tgt)
        except KeyError:
            connections[src] = [tgt]
    src_station = 1
    while len(route) < len(connections):
        route.append(cur_station)
        tgt_stations = connections[src_station]
        for tgt_station in connections[src_station]:
